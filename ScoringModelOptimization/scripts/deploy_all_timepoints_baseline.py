#!/usr/bin/env python3
"""
Deploy BASELINE (no temperature scaling) to ALL timepoints.

For each timepoint T1, T2, ..., T14/T15:
- Create pairs: (T0, Ti)
- Generate predictions using RAW model outputs + confidence weighting
- Track temporal evolution of phenotype

This provides per-neuron predictions for comparison with temperature-scaled approach.
"""
...
def deploy_timepoint(model, dataset, device='cuda'):
    """Deploy model for a specific timepoint (BASELINE - no temperature scaling)"""
    model.eval()
    model.to(device)
    ...
    # Baseline: raw softmax (NO temperature scaling)
...
def main():
    print("\n" + "="*80)
    print("DEPLOYING BASELINE (NO TEMPERATURE SCALING) TO ALL TIMEPOINTS")
    print("="*80)
    print("\nApproach: Raw predictions + Confidence Weighting")
    print("="*80)
    ...
    output_dir = Path('ScoringModelOptimization/results/temporal_predictions_baseline')
    ...
    for plate_name in ['plate1', 'plate2']:
        ...
        for timepoint in range(1, final_timepoint + 1):
            ...
            # Deploy
            neuron_preds, well_tile_preds = deploy_timepoint(model, dataset)
    ...
    print("\nBaseline (no temperature scaling) predictions for all timepoints!")
    print("="*80 + "\n")
