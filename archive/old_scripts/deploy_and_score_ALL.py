#!/usr/bin/env python3
"""Deploy models on ALL data (not just test split)"""
import sys
...
# Try importing from wells version first, fallback to regular
try:
    from train_lightning_wells import AggregationClassifier
except ImportError:
    from train_lightning import AggregationClassifier

class SimpleDataset(Dataset):
    """Load all images without train/test split"""
...
def deploy_all(checkpoint_path, metadata_path, root_dir, channel, output_file):
    """Deploy on ALL data"""
    print(f"Deploying {channel}...")
    
    # Load model - try with just num_classes first
    try:
        model = AggregationClassifier.load_from_checkpoint(checkpoint_path, num_classes=2)
    except TypeError:
        # If that fails, try with class_names
        model = AggregationClassifier.load_from_checkpoint(
            checkpoint_path, num_classes=2, class_names=['class0', 'class1']
        )
    ...
    print(f"✅ Saved {len(metadata)} samples to {output_file}")
    return metadata
...
deploy_all(rev_rfp1, metadata, root_dir, 'RFP1', output_dir / 'reversion_rfp1_ALL.csv')
deploy_all(rev_halo, metadata, root_dir, 'Halo', output_dir / 'reversion_halo_ALL.csv')
deploy_all(resc_rfp1, metadata, root_dir, 'RFP1', output_dir / 'rescue_rfp1_ALL.csv')
deploy_all(resc_halo, metadata, root_dir, 'Halo', output_dir / 'rescue_halo_ALL.csv')
print("✅ ALL DONE")
